<template>
  <div
    class="my-perm-group-perm iam-group-perm-wrapper"
    v-bkloading="{ isLoading: pageLoading, opacity: 1 }"
  >
    <div class="mb20 iam-group-perm-button">
      <bk-button
        theme="primary"
        @click="handleBatchAddUserGroup"
        data-test-id="group_btn_create"
      >
        {{ $t(`m.permTemplate['添加用户组']`) }}
      </bk-button>
      <bk-button :disabled="!currentSelectGroupList.length" @click="handleBatchQuit">
        {{ $t(`m.common['批量退出']`) }}
      </bk-button>
      <bk-button
    ...
    >
      <div
        class="attach-action-preview-content-wrapper"
      ...
      >
        <template v-if="!isLoading">
          ...
          <p class="user-group-error" v-if="isShowGroupError">
        ...
        <section ref="expiredAtRef" class="mt20">
          <iam-deadline
            ...
            :cur-role="curRole"
          />
          <p class="expired-at-error" v-if="isShowExpiredError">
        ...
        <section class="button-warp">
          ...
          <bk-button
            class="mb20"
            theme="default"
            @click="handleBatchUserGroupCancel"
            data-test-id="group_btn_create"
          >
            {{ $t(`m.common['取消']`) }}
          </bk-button>
    ...
    methods: {
      ...
      async fetchPermGroups (isTableLoading = false, isPageLoading = false) {
        try {
          ...
          const { code } = e;
          this.currentSelectGroupList = [];
          this.groupPermEmptyData = formatCodeData(code, this.emptyData);
      ...
      fetchSelectedGroups (type, payload, row) {
        const typeMap = {
          ...
          all: () => {
            ...
            const selectGroups = this.currentSelectGroupList.filter(
      ...
      handleDeleteActions (type) {
        const typeMap = {
          quit: () => {
            ...
            const adminGroups = this.currentSelectGroupList.filter(
              (item) =>
                item.attributes
      ...
      async handleSubmitAdd (payload) {
        ...
        // 判断批量选择的用户组里是否包含管理员组
        const hasAdminGroups = this.currentSelectGroupList.filter(
          (item) =>
        ...
        if (hasAdminGroups.length) {
          ...
          this.messageWarn(
            this.$t(`m.info['用户组为管理员组，不能添加部门']`, {
              value: `${this.$t(`m.common['【']`)}${adminGroupNames}${this.$t(
                `m.common['】']`
              )}`
            }),
            3000
      ...
      async fetchUserGroupList () {
        ...
        try {
          ...
          this.emptyDialogData = formatCodeData(
            code,
